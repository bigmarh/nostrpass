...
export class NostrPassButton {
    constructor(embassy, config = {}) {
        ...
        this.isCheckingAuth = true; // Track if we're still checking auth status
            ...
            if (this.currentUser) {
                ...
                try {
                    ...
                    if (activeIdentityIndex !== undefined && activeIdentityIndex !== this.currentUser.identityIndex) {
                        ...
                        const newActiveIdentity = allIdentities.find((id) => id.index === activeIdentityIndex);
                    ...
                    const currentIdentity = allIdentities.find((id) => id.index === this.currentUser.identityIndex);
    ...
    async renderUserButton() {
        ...
        const hasAuthorizedIdentity = user.authorized || allIdentities.some((id) => id.isAuthorized);
    ...
    `;
        ...
        const userBtn = this.container.querySelector('[data-action="toggle-menu"]');
        ...
        switchBtns.forEach(btn => {
            btn.addEventListener('click', (e) => {
                const identityIndex = parseInt(e.currentTarget.getAttribute('data-identity-index') || '0');
        ...
        const manageBtn = this.container.querySelector('[data-action="manage-account"]');
        manageBtn?.addEventListener('click', () => {
            this.closeDropdown();
            this.handleManageAccount();
        });
        const signOutBtn = this.container.querySelector('[data-action="sign-out"]');
    ...
    async handleSwitchIdentity(identityIndex) {
        this.closeDropdown();
        try {
            ...
            if (error?.message?.includes('not authorized')) {
                try {
                    const result = await this.embassy.manageAccount({
                        forcePrompt: true,
                        buttonElement: this.container.querySelector('[data-action="toggle-menu"]')
                    });
                    if (result?.identity) {
                        this.currentUser = {
                            identityIndex: result.identityIndex,
    ...
    async toggleDropdown() {
        ...
        try {
            ...
            if (authStatus?.isLocked) {
                ...
                const btn = this.container.querySelector('[data-action="toggle-menu"]');
        ...
        const dropdown = this.container.querySelector('[data-dropdown]');
        if (!dropdown)
            return;
    ...
    async handleNotAuthorizedClick() {
        try {
            const btn = this.container.querySelector('[data-action="toggle-menu"]');
            ...
            const handleSelected = async (e) => {
                ...
                if (ce.detail.requestId === requestId) {
                    ...
                    try {
                        const response = await this.embassy.getAllIdentities();
                        const allIdentities = response?.identities || [];
                        const selectedIdentity = allIdentities.find((id) => id.index === ce.detail.identityIndex);
                        if (selectedIdentity) {
                            this.currentUser = {
            ...
            const cleanup = () => {
                window.removeEventListener('account-picker-selected', handleSelected);
                window.removeEventListener('account-picker-rejected', handleRejected);
            };
            window.addEventListener('account-picker-selected', handleSelected);
            window.addEventListener('account-picker-rejected', handleRejected);
