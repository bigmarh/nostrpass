...
/**
 * Get the worker URL for the current context.
 * In extension context, uses chrome.runtime.getURL().
 * In web context, uses standard URL resolution.
 */
function getWorkerUrl(): string {
  if (isExtensionContext()) {
    return chrome.runtime.getURL('crypto.worker.js');
  }
  // Web context fallback
  return import.meta.env.PROD
    ? new URL('/crypto.worker.js', window.location.origin).href
    : new URL('../workers/crypto.worker.ts', import.meta.url).href;
}

/**
 * Detect the best worker backend for the current environment.
 * - Chrome extensions use Dedicated Worker (SharedWorker has limitations in extensions)
...
export function detectBestBackend(): WorkerBackend {
  // Chrome extensions should use dedicated workers for reliability
  if (isExtensionContext()) {
...
/**
 * Create a Dedicated Worker client (extension/mobile).
 * Dedicated Worker runs per-popup - no cross-tab session sharing.
 * In extensions, this is the most reliable approach.
 */
function createDedicatedWorkerClient(): any {
  try {
    const workerUrl = getWorkerUrl();
    console.log('[NostrPass] Creating dedicated worker from:', workerUrl);
    ...
    console.log('[NostrPass] Crypto: using Dedicated Worker');

    return createWorkerClient(dedicatedWorker, {
      timeout: 60000,
      onError: (err) => {
        console.error('[NostrPass] Worker error:', err);
      },
    });
  } catch (e1) {
    console.warn('[NostrPass] Module Dedicated Worker failed:', e1);

    // Fallback: Classic worker (non-module)
    try {
      const fallbackUrl = isExtensionContext()
        ? chrome.runtime.getURL('crypto.worker.js')
        : '/crypto.worker.js';

      dedicatedWorker = new Worker(fallbackUrl, {
