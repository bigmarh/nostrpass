...
export class NostrPassButton {
  ...
  constructor(embassy: any, config: NostrPassButtonConfig = {}) {
    ...
    document.addEventListener('click', (e) => {
      if (this.dropdownVisible && !this.container.contains(e.target as Node)) {
        this.closeDropdown();
      }
    });
  }
  ...
  private renderUserButton() {
    ...
    const toggleBtn = this.container.querySelector('[data-action="toggle-dropdown"]') as HTMLButtonElement;
    toggleBtn.addEventListener('click', () => this.toggleDropdown());

    const manageBtn = this.container.querySelector('[data-action="manage"]') as HTMLButtonElement;
    if (manageBtn) {
      manageBtn.addEventListener('click', () => this.handleManageAccount());
    }

    const signoutBtn = this.container.querySelector('[data-action="signout"]') as HTMLButtonElement;
